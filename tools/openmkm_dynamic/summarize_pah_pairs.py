"""Report the paired CRECK/Aramco batch: the light layer, then the heavy pool.
...
nothing about which is right. Second, how much carbon CRECK carries past C6,
which Aramco has no channel for and therefore reports as zero by construction.
"""
import argparse
import json
...
MECH = ROOT / "tools/cantera/mechanisms/creck2003-ht-soot-nox.yaml"
...
def main():
    ...
    counts = carbon_counts()

    analysis, lines = [], []
    for row in rows:
        creck = row["creck"]["metrics"]
    ...
    def block(entry):
        ...
        out += ["", "| Above C6, % of inlet C | Aramco | CRECK |", "|---|---|---|",
        ...
        out += [f"| BIN lumps | {pct(ha['BIN'])} | {pct(h['BIN'])} |",
                ...
                "Largest CRECK carriers above C6: "
                + ", ".join(f"{s} {pct(v)} %" for s, v in h["top"] if v > 0) + ".", ""]
        return out

    lines += ["# Paired PAH comparison: CRECK 2003 HT+soot against the archived Aramco map", "",
              ...
              "reactor. The heavy pool is CRECK's alone: AramcoMech 2.0 has no species above "
              "C8 and no ring above benzene, so its column there is zero by construction and "
