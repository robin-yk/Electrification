...
def main():
    ...
    try:
        if a.stage=='propose':
            assert not D.exists()
            subprocess.run([sys.executable,str(Path(__file__).with_name('propose_rph_grid_bo.py')),'--round','2'],check=True,timeout=120)
        else:
            ...
            for n in [400,800]:
                with (out/f'n{n}.log').open('w') as log:
                    subprocess.run([sys.executable,__file__,'--samples',str(n)],stdout=log,stderr=subprocess.STDOUT,check=True,timeout=max(1,min(140,cap-(time.monotonic()-start))))
