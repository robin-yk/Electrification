...
def run(name,cold=False,tight=False,T_C=1750.,flow=50.,ch4=.5):
    start=time.monotonic(); T=300 if cold else T_C+273.15
    gas=ct.Solution('gri30.yaml' if cold else str(MECH))
    gas.TPX=T,101325,{'CH4':ch4,'CO2':1-ch4}
    mw=gas.molecular_weights; y0=gas.Y.copy()
    atoms=np.array([[gas.n_atoms(k,e) for k in range(gas.n_species)] for e in ('C','H','O')])
    mdot=flow/22414/60*gas.mean_molecular_weight/1000
    ...
    ac=float(m[gas.species_index('C2H2')])*flow/22414*60*1000
    rad=.57*5.670374419e-8*.000608*(T**4-298.15**4)
    qgas=pc.mass_flow_rate*r.thermo.enthalpy_mass-mdot*hin
    data=dict(name=name,T_K=T,flow_sccm=flow,feed_CH4=ch4,volume_m3=r.volume,
    ...
    hout=r.thermo.enthalpy_mass
    gas.TPY=298.15,101325,y
    qrx=pc.mass_flow_rate*gas.enthalpy_mass-mdot*hin
    qsens=pc.mass_flow_rate*(hout-gas.enthalpy_mass)
    assert abs(qrx+qsens-qgas)<1e-7
    data.update(net_reaction_heat_298_W=qrx,outlet_sensible_W=qsens,
                yield_C2H2_pct=200*float(m[gas.species_index('C2H2')]),
                radiation_remaining=.1,scenario_input_W=qgas+.1*rad,
                reaction_heat_fraction_pct=100*qrx/(qgas+.1*rad))
