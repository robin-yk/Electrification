...
def main():
    ...
    ap.add_argument('--yield-objective',action='store_true')
    ...
    assert any(b['id']==batch and b['reserved_s']==150 for b in ledger['batches'])
    ...
    try:
        for active in ['inert','anchor','candidate','refined']:
            ...
            with (out/(active+'.log')).open('w') as f:
                cmd=[sys.executable,__file__,'--output-dir',str(out),'--worker',active]
                if a.yield_objective:cmd+=['--yield-objective']
