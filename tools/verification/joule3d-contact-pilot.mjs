...
const outPath=new URL('../../docs/research/joule-v6-2026-09-07/3d-contact-pilot-aligned.json',import.meta.url);
...
const base={shape:'rod',meshType:'cartesian',length:30,width:12,height:12,bore:0,rho:1.45e-6,alpha:0,k:11,density:7100,cp:460,mode:'P',command:30,vmax:40,imax:375,pmax:15000,ambient:25,sink:25,h:15,emissivity:.8,hc:0,maxTemp:1500,study:'steady',initial:25,flow:false,wall:false,contactR:0,thermalR:0,slew:0,jlimit:0,electrodeLength:0,limitAction:'limit',rhoCurve:[],kCurve:[],cpCurve:[]};
try{
 for(const n of [12,18])for(const layout of ['aligned']){
  context.p={...base,n,nx:n,ny:n,nz:Math.round(n*2.5),contact:50,offsetA:-50,offsetB:-50};
  const r=vm.runInContext('solveModel(p)',context,{timeout:150000});
  if(Math.abs(r.stats.P-30)>1e-6||r.stats.energyError>1e-5||r.stats.chargeError>1e-5)throw Error('Power/energy/charge gate '+JSON.stringify(r.stats));
