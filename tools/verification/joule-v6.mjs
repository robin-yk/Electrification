...
import {defaultInput,radialParabola,annulusDrops,mmsStudy} from './joule.mjs';
import {run as literatureCheck} from './crosscheck.mjs';
const dir=new URL('../../docs/research/joule-v6-2026-09-07/',import.meta.url);fs.mkdirSync(dir,{recursive:true});
const file=new URL('data-v6-final.json',dir);if(fs.existsSync(file))throw Error('Preserve existing run; inspect its status before resuming.');
...
try{
 ...
 out.timeDifference=Math.max(...['heat','cool'].map(p=>Math.abs(out.transients.find(t=>t.dt===2.5&&t.phase===p).avg-out.transients.find(t=>t.dt===1.25&&t.phase===p).avg)));
 ...
 out.materials=[];
 for(const [name,lo,hi] of [['SiC',1,100],['SiSiC (Si-infiltrated SiC)',1,100],['Kanthal A-1 (FeCrAl)',10,10000],['CFP',1,100]]){
  const m=MATERIALS.find(m=>m.name===name);if(!m)throw Error('Missing material '+name);
 ...
 check();out.verification={radial:radialParabola(10),annulus:annulusDrops(10),mms:mmsStudy(3)};save();
 check();out.literature=literatureCheck();save();
